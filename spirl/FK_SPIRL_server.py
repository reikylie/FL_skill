...
if __name__ == "__main__":
    ...
    init_path = "/home/workspace/skill/experiments/skill_prior_learning/kitchen/wrong_prior"
    
    init_model1 =  Custome_train2.ModelTrainer(args=config,cid=0)
    init_model2 =  Custome_train2.ModelTrainer(args=config,cid=0)
    init_model1.model.load_state_dict(torch.load("/home/workspace/skill/experiments/skill_prior_learning/kitchen/test_equal/weights/weights_ep03.pth")['state_dict'])
    init_model2.model.load_state_dict(torch.load("/home/workspace/skill/experiments/skill_prior_learning/kitchen/test_equal/weights/weights_ep04.pth")['state_dict'])
    key_value = init_model2.model.state_dict().keys()
    init_dict1 = init_model1.model.state_dict()
    init_dict2 = init_model2.model.state_dict()
    for d in key_value :
        if not (torch.equal(init_dict1[d], init_dict2[d])):
            print(d)
    print("done")
    #state = {
    #    'epoch': 99,
    ...
    #}
    #save_path = os.path.join("/home/workspace/skill/experiments/skill_prior_learning/kitchen/test_equal","weights")
    #os.makedirs(save_path,exist_ok=True)
    #torch.save(state, save_path+"/weights_ep04.pth")
    '''
    ...
    base_model =  Custome_train2.ModelTrainer(args=config,cid=0)
    ...
    torch.device('cuda')
    normal_state_dict = base_model.model.state_dict()
    init_state_dict = init_model.model.state_dict()
    #params_dict = zip(init_model.model.state_dict().keys(), load_dict)
    #state_dict = OrderedDict({k: torch.Tensor(load_dict[v]) for k, v in params_dict})
    #load_dict.close()
    l = []
    for d in normal_state_dict :
        if "decoder" in d :
            normal_state_dict[d] = init_state_dict[d]
    init_model.model.load_state_dict(normal_state_dict,strict = True)
    #init_model.val()
    ...
    save_path = os.path.join("/home/workspace/skill/experiments/skill_prior_learning/kitchen/wrong_prior","weights")
    os.makedirs(save_path,exist_ok=True)
    torch.save(state, save_path+"/weights_ep99.pth")
