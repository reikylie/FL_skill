    ...
    def val(self):
        ...
        for j in range(7):
            ...
            with self.agent.val_mode():
                with torch.no_grad():
                    with timing("Eval rollout time: "):
                        for i in range(100):   # for efficiency instead of self.args.n_val_samples
                            val_rollout_storage.append(self.sampler.sample_episode(is_train=False, render=False, task_num = j))
            ...
            if self.is_chef:
                ...
                reward.append(rollout_stats.avg_reward)
...
if __name__ == '__main__':
    config = AttrDict(
        path = "spirl/configs/hrl/kitchen2/spirl_cl",
        prefix = "normal_decoder_1",
        new_dir = False,
        dont_save = True,
        resume = "39",
        mode = 'val',
